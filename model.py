...
class EmbeddingLayer(nn.Module):
    ...
    def _get_embedding(self):
        if self.use_normalized_embedding:
            ...
            return _normalize_embedding_layernorm(weight)
            # return _normalize_embedding_layernorm(self.em?bedding)
...
class LangFlow(transformers.PreTrainedModel):
    ...
    def _forward_diffusion(self, x_embed: torch.Tensor, 
                           gamma: torch.Tensor, noise=None) -> torch.Tensor:
    ...
    def compute_loss_condition(self, x, y, t, training_prob=0.25):
        ...
        z = self._forward_diffusion(x0, gamma)
        ...
        if training_prob <= 0:
            ...
            if training_prob >= 1:
                ...
                active_mask = (torch.rand(batch_size, device=x.device) < training_prob)

            x_self_cond = torch.zeros_like(x0)
            active_idx = active_mask.nonzero(as_tuple=True)[0]

            if active_idx.numel() > 0:
                z_active = z.index_select(0, active_idx)
                gamma_active = gamma.index_select(0, active_idx)

                with torch.no_grad():
                    logits_active = self.forward(
                        noisy_embeds=z_active,
                        timesteps=gamma_active,
                        x_self_cond=None,
                        return_dict=False,
                    )
                    probs_active = F.softmax(logits_active, dim=-1, dtype=torch.float32)
                    self_cond_active = self._embed_tokens(probs_active)
                    del logits_active, probs_active

                self_cond_active = self_cond_active.to(dtype=x_self_cond.dtype)
                x_self_cond.index_copy_(0, active_idx, self_cond_active)
                del (z_active, gamma_active, self_cond_active)

            del active_mask, active_idx
        ...
        weight = torch.pow(1 + alphas / sigmas, 2)
        loss = (weight * ((x_pred - x0) ** 2).reshape(x0.shape[0], -1).mean(dim=1)).mean()
        return loss